...
public class Sample3 : SingletonMono<Sample3> {
	...
	public void Reload(){
		...
		// Begin Reset without tween
		Scrollbar scroll=verticleScroll.gameObject.GetComponent<ScrollRect> ().horizontalScrollbar;
		if(scroll!=null){
			scroll.value=1;// or 0 if invert scroll
		}
		scroll=verticleScroll.gameObject.GetComponent<ScrollRect> ().verticalScrollbar;
		if(scroll!=null){
			scroll.value=1;// or 0 if invert scroll
		}
		// end reset without tween.
